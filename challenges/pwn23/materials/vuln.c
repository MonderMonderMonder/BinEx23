...
size_t get_serial_key() {
	void * tmp = malloc(0x1000);
	size_t r = ((((size_t)tmp + rand() % 0x999)<< 32) + rand() % 0x99999999) ^ 0x7faaaabbbbaaaaf7;
	free(tmp);
	return r;
}
...
int main()
{
    ...
    printf("\x1b[35mToday's special: Enjoy our key-value store's free\x1b[30m*\x1b[35m trial (Serial key: %lx)!\x1b[0m\n", get_serial_key());
